...
In particular, an application of this theorem is that the $n$-th roots of unity ($n \in \mathbb{N}$) are given by

$$
\omega_k = e^{\frac{2k\pi i}{n}} = w_1^k \qquad k = 1, 2, \ldots, n-1, \omega_1 = e^{\frac{2 \pi i}{n}}
$$

which essentially says that there are $n$ roots to the complex number that satisfies the equation $\omega^n = 1$. From this, we can generalize this as 

$$\sum_{k=0}^{n-1} \omega_k = \sum_{k=0}^{n-1} \omega^k_1 = 0$$

where $\omega$ is any $n$-th root of unity except for $\omega = 1$.
